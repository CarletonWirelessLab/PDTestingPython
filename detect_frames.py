...
def detect_frames(complex_data):
    ...
    # CONSTANTS:
    # short training sequence lenght
    sts_len = c.sts_len
    ...
    for m in range(0, pkt_count):
        ...
        # and move left until you hit the start of the pakcet or detect some engergy
        if m == pkt_count-1:
            end_index = s_len-1
        else:
        ...
        for n in range(end_index, start_index, -1):
            if abs(complex_data[n]) > 2 * threshold:
                pkt_end.append(n)
                break
    # pkt_start = [pkt_start[x] for x in np.arange(0, pkt_count)]
    pkt_start_array = np.array(pkt_start)
    # pkt_end = [pkt_end[x] for x in np.arange(0, pkt_count)]
